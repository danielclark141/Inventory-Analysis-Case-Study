...
-- Creating a new temporary table to store inventory levels by date and stocking status
...
-- Using a temp table to identify the last transaction date by each inventory id.
DROP TEMPORARY TABLE IF EXISTS temp_last_transaction_date;
CREATE TEMPORARY TABLE temp_last_transaction_date (
	id						INT 		PRIMARY KEY	AUTO_INCREMENT,
    Inventory_id			VARCHAR(75)	NOT NULL,
    Last_Transaction_Date	DATE		NOT NULL
);

INSERT INTO temp_last_transaction_date (Inventory_id, Last_Transaction_Date)
SELECT
	Inventory_id,
...
-- Comparing my calculated ending inventory to the ending inventory from the original dataset to identify any mismatches.
SELECT 
	COUNT(t.Inventory_id) AS Ending_Inventory_Mismatches
FROM temp_inventory_level t
INNER JOIN temp_last_transaction_date t2
	ON t.Inventory_id = t2.Inventory_id
INNER JOIN ending_inventory ei
	ON t.Inventory_id = ei.Inventory_id
WHERE t.Transaction_Date = t2.Last_Transaction_Date AND t.Ending_Inventory != ei.On_Hand_Inventory;
